...
def create(runcard_path=RUNCARD):
    ...
    channels = ChannelMap()
    # Readout
    channels |= Channel(name="feedin", port=modules["qrm_rf0"].ports["o1"])
    # Feedback
    channels |= Channel(name="feedback", port=modules["qrm_rf0"].ports["i1"])
     # Drive
    channels |= Channel(name="drive1", port=modules["qcm_rf0"].ports["o1"])
    channels |= Channel(name="drive2", port=modules["qcm_rf0"].ports["o2"])
    ...
    for q in range(0, 3):  # q0, q1
        qubits[q].readout = channels["feedin"]
        qubits[q].feedback = channels["feedback"]
