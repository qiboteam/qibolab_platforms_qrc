...
def create():
    ...
    channels |= Channel(name="feed_back", port=modules["qrm_rf0"].ports("i1",out=False))
     ...
     # Drive
    channels |= Channel(name="drive", port=modules["qcm_rf0"].ports("o1")) # qubit 1 and 2
    channels |= Channel(name="dummy", port=modules["qcm_rf0"].ports("o2"))
    ...
    # get the qubit drive isntrument
   
    try:
         channels['feed_in'].port.nco_phase_offs= 0
