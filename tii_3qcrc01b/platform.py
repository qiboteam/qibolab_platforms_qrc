...
def create():
    ...
    modules = {
        ...
        "qrm_rf1": QrmRf("qrm_rf1", f"{ADDRESS}:6"),  # q3
    ...
    channels |= Channel(name="feed_back", port=modules["qrm_rf0"].ports("i1",out=False))
     ...
     # Drive
    channels |= Channel(name="drive3", port=modules["qrm_rf1"].ports("o1")) # qubit 2
    channels |= Channel(name="drive1", port=modules["qcm_rf0"].ports("o1")) # qubit 1
    channels |= Channel(name="drive2", port=modules["qcm_rf0"].ports("o2")) # qubit 3
